...
for image_name in os.listdir(RGB_sal):
    if os.path.exists(save_path + image_name):
        ...
        sal2 = cv2.imreadsal2 = cv2.imread( RGBD_sal + image_name)
        for class_label in all_classes:
            coordinates = get_coordinates_for_class(label, class_label)  # 区域坐标
            score_RGB = get_score_by_id(RGB_score+image_name+'.txt', class_label)
            score_depth = get_score_by_id(d_score+image_name+'.txt', class_label)
            #print(class_label,score_RGB,score_depth)
            sal2 = apply_color_to_regions(sal1,sal2, coordinates, score_RGB, score_depth)
