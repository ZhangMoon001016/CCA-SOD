...
class DCAM(nn.Module):
    ...
    def forward(self, x, y):
        ...
        xx = torch.mul(self.sftmax(xx) * xx.shape[1], x)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.SUM1 =DCAM(512,512,64)
        self.SUM2 =DCAM(512,256,64)
        self.SUM3 =DCAM(256,128,64)
        self.SUM4 =DCAM(128,64,64)

        self.aspp = ASPP_2468(64,64)
    ...
    def forward(self,s1,s2,s3,s4,x1,x2,x3,x4,randx):
        ...
        for i in range(4):
            if i != randx:
                with torch.no_grad():
                    s5_ = self.aspp(s4, i)
                    s5_ = self.relu(self.bn0(self.conv0(s5_)))
                    s5_ = F.interpolate(s5_, size=s4.size()[2:], mode='bilinear', align_corners=True)
                    s4_ = self.relu(self.bn1(self.conv1(s4 + s5_)))
                    s4_ = F.interpolate(s4_, size=s3.size()[2:], mode='bilinear', align_corners=True)
                    s3_ = self.relu(self.bn2(self.conv2(s4_ + s3)))
                    s3_ = F.interpolate(s3_, size=s2.size()[2:], mode='bilinear', align_corners=True)
                    s2_ = self.relu(self.bn3(self.conv3(s3_ + s2)))
                    s2_ = F.interpolate(s2_, size=s1.size()[2:], mode='bilinear', align_corners=True)
                    s1_ = self.relu(self.bn4(self.conv4(s2_ + s1)))
                    s.append(s1_)
            else:
                s5_ = self.aspp(s4, i)
                s5_ = self.relu(self.bn0(self.conv0(s5_)))
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers-1):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               input_resolution=(patches_resolution[0] // (2 ** i_layer if i_layer < self.num_layers - 2 else 4),
                                                 patches_resolution[1] // (2 ** i_layer if i_layer < self.num_layers - 2 else 4)),
...
class BBNet(nn.Module):
    def __init__(self, cfg=None):
        ...
        self.cfg      = cfg
        self.bkbone   = SwinTransformer(224)
        self.resnet    = resnet18()
        self.bkbone.load_state_dict(torch.load('E:/BBRF-TIP-master/model/swin_224.pth'),strict=False)
        self.resnet.load_state_dict(torch.load('E:/BBRF-TIP-master/model/resnet18.pth'),strict=False)
        self.decode   = Decoder()
    ...
    def forward(self, x,shape=None,mask=None):
        ...
        s1,s2,s3,s4 = self.bkbone(y)
        if self.cfg.mode == 'train':
            randx = np.random.randint(0,4)
            (p0,p1,p2,p3),(pred2,pred3,pred4) = self.decode(s1,s2,s3,s4,x1,x2,x3,x4,randx)
            pred = []
            if randx == 0:
                pred.append(F.interpolate(self.linearr1_88(p0), size=shape, mode='bilinear'))
            else:
                with torch.no_grad():
                    pred.append(F.interpolate(self.linearr1_88(p0), size=shape, mode='bilinear'))
            if randx == 1:
                pred.append(F.interpolate(self.linearr1_80(p1), size=shape, mode='bilinear'))
            else:
                with torch.no_grad():
                    pred.append(F.interpolate(self.linearr1_80(p1), size=shape, mode='bilinear'))
            if randx == 2:
                pred.append(F.interpolate(self.linearr1_72(p2), size=shape, mode='bilinear'))
            else:
                with torch.no_grad():
                    pred.append(F.interpolate(self.linearr1_72(p2), size=shape, mode='bilinear'))
            if randx == 3:
                pred.append(F.interpolate(self.linearr1_64(p3), size=shape, mode='bilinear'))
            else:
                with torch.no_grad():
                    pred.append(F.interpolate(self.linearr1_64(p3), size=shape, mode='bilinear'))
            B,C,W,H = x.shape
            with torch.no_grad():
                w = torch.zeros(B,1,W,H).cuda()
                for i in range(4):
                    if i == ((randx+3)%4):
                        w = w+F.binary_cross_entropy_with_logits(pred[i], mask,reduction='none')
            ma = torch.max(w)
            mi = torch.min(w)
            #print(w.max())
            if ma > mi:
                ...
                w = w+1
            pred2 = F.interpolate(self.linear1(pred2), size=shape, mode='bilinear')
            pred3 = F.interpolate(self.linear2(pred3), size=shape, mode='bilinear')
            pred4 = F.interpolate(self.linear3(pred4), size=shape, mode='bilinear')
            return pred[randx],pred2,pred3,pred4,w
        else:
            (p0,p1,p2,p3),_ = self.decode(s1,s2,s3,s4,x1,x2,x3,x4,-1)
            pred1 = F.interpolate(self.linearr1_88(p0), size=shape, mode='bilinear')
            pred2 = F.interpolate(self.linearr1_80(p1), size=shape, mode='bilinear')
            pred3 = F.interpolate(self.linearr1_72(p2), size=shape, mode='bilinear')
            pred4 = F.interpolate(self.linearr1_64(p3), size=shape, mode='bilinear')
