...
from utils.recorder import MsgLogger
import time

from utils.recorder import AvgMeter,  MsgLogger, TimeRecoder#CalTotalMetric,
...
@torch.no_grad()
def eval_once(model, data_loader, save_path="E:/data/11/", show_bar=True):
    model.eval()
...
def testing(model, msg_logger, cfg):
    ...
    for te_data_name in cfg.data.test.name:
        ...
        cfg.path.save='E:/data/11/'
        if not os.path.exists(cfg.path.save):
            os.makedirs(cfg.path.save)
        print("cfg.path.save:",cfg.path.save)
...
def parse_config():
    ...
    parser.add_argument("--config", type=str, default='./configs/rgbd-3dataset.py',required=True)
    parser.add_argument("--output-root", type=str, default="F:/zmy/paper2/data/STERE/CAVER_Sal")
    parser.add_argument("--model-name", type=str,default='CAVER_R101D', required=True)
    parser.add_argument("--load-from", type=str,default='E:/CAVER-main/pth/caver-r101d-rgbd-njudnlpr.pt')
    parser.add_argument("--pretrained", type=str, help="Pretrained params of the backbone of your model.")
    parser.add_argument("--info", type=str,default='rgbd-3dataset')
